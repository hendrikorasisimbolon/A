 ...
 */
    (private val activity: AppCompatActivity, private val movieItems: MutableList<ItemExpedisi>) : BaseAdapter() {
    private var inflater: LayoutInflater? = null
    private val listlokasiasli: ArrayList<ItemExpedisi>
    ...
    fun filter(charText: String) {
        ...
        if (charText.length == 0) {
            ...
            for (lok in listlokasiasli) {
                if (lok.name?.toLowerCase()?.contains(charText)!!) {
                    movieItems.add(lok)
                } else {
                }
    ...
    fun setList(movieItems: List<ItemExpedisi>) {
